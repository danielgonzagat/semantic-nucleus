...
- O `MultiOntologyManager` embutido ativa domínios `core`, `code`, `medical`, `legal` e `finance`, infere quais deles aparecem no texto/estrutura corrente e grava um nó `ontology_scope` no contexto/meta_summary com lista de domínios ativos/inferidos, contagem total de relações e digest BLAKE2b do escopo — fechando a auditoria “entrada → ontologia usada → resposta” em nível de domínio.
