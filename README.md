...
- Código Python agora é convertido em meta-AST auditável (`nsr.code_ast.build_python_ast_meta`), anexado ao contexto/meta_summary tanto na rota CODE quanto no fallback textual quando detectamos dialeto de código: o nó `code_ast` carrega `node_count`, `summary` e o tronco completo do AST (limitado e marcado quando truncado). Para matemática, `nsr.math_ast.build_math_ast_node` transforma instruções do Math-Core em `math_ast` (operador, expressão, operandos, valor) e o MetaTransformer publica esses campos (`math_ast_*`) em toda resposta matemática.
