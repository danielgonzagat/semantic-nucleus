...
- A rota estatística `MetaRoute.STAT` consome comandos `BAYES {json}`: o payload descreve variáveis/cpts discretas, o núcleo monta uma rede bayesiana determinística (`nsr.bayes_engine.BayesNetwork`), executa enumeração exata e retorna o posterior como meta-estrutura + plano ΣVM direto (`stat_direct_answer`), tudo auditável no `meta_summary`.
