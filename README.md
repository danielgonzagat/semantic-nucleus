...
- A rota estatística `MetaRoute.STAT` aceita comandos `BAYES {json}` (redes bayesianas discretas) e `MARKOV {json}` (cadeias/hmms determinísticos). Ambos os payloads descrevem estruturas probabilísticas completas, o núcleo executa a inferência exata (`nsr.bayes_engine.BayesNetwork` ou `nsr.markov_engine.MarkovModel`) e retorna o resultado como LIU + plano ΣVM direto (`stat_direct_answer`) com rastreabilidade total no `meta_summary`.
