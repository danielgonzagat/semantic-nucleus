...
- A rota estatística `MetaRoute.STAT` aceita comandos `BAYES {json}` (redes bayesianas discretas), `MARKOV {json}` (cadeias/hmms determinísticos), `REGRESS {json}` (regressão linear múltipla) e `FACTOR {json}` (belief propagation em grafos fatoriais). Cada payload descreve a estrutura completa, o núcleo roda o motor correspondente (`nsr.bayes_engine`, `nsr.markov_engine`, `nsr.regression_engine`, `nsr.factor_graph_engine`) e devolve LIU + plano ΣVM direto (`stat_direct_answer`) auditável no `meta_summary`.
...
- O `MultiOntologyManager` embutido ativa domínios `core`, `code`, `medical`, `legal`, `finance` **e agora toda a Ontologia Universal v1.0 (categorias 1–10)**, infere quais deles aparecem no texto/estrutura corrente e grava um nó `ontology_scope` no contexto/meta_summary com lista de domínios ativos/inferidos, contagem total de relações e digest BLAKE2b do escopo — fechando a auditoria “entrada → ontologia usada → resposta” em nível de domínio.
...
- Probabilidades contextuais determinísticas (`context_probabilities`) acompanham cada meta_summary: contagens normalizadas de relações, tags de contexto e goals são calculadas a partir do ISR. Essas distribuições também priorizam deterministamente a fila Φ, orientando qual operador roda primeiro sem recorrer a pesos ocultos.
- MetaPlanner e síntese formal funcionam de ponta a ponta sem intervenção manual: `detect_plan_goal` injeta `Φ_PLAN_DECOMPOSE`, o runtime observa quando um `plan_decompose` surge e enfileira `Φ_SYNTH_PLAN` automaticamente (apenas se ainda não existir `synth_plan` com o mesmo digest). Consultas lógicas (`QUERY ...`) executam `Φ_PROVE` no final do loop, e sempre que um `logic_proof` novo aparece o núcleo agenda `Φ_SYNTH_PROOF`. Ambos operadores gravam `source_digest`/`proof_digest` e relações `plan/SYNTHESIZED`/`logic/SYNTH`, permitindo auditar a trilha “plano → síntese” e “prova → síntese” diretamente no `meta_summary`.
- `Φ_SYNTH_PROG` segue a mesma lógica determinística: sempre que `REWRITE_CODE` gera um `code_ast_summary`, o runtime detecta resumos ainda não sintetizados (via digest) e agenda `SYNTH_PROG` automaticamente. O operador escreve `source_digest` no `synth_prog` e na relação `code/SYNTH`, garantindo a mesma rastreabilidade aplicada a planos e provas.
- Um novo bloco `meta_synthesis` é emitido junto ao `meta_summary`, consolidando todos os `synth_plan`, `synth_proof` e `synth_prog` presentes no contexto. Cada entrada replica o digest de origem (`source_digest`/`proof_digest`), contagem de passos, idioma/programa e status, além dos totais por categoria (`synthesis_plan_total`, `synthesis_proof_total`, `synthesis_program_total`). A API (`meta_summary_to_dict`) expõe essas listas como `synthesis_plans`, `synthesis_proofs` e `synthesis_programs`, fechando o elo auditoria → síntese.
- A `meta_memory` também absorve esses campos: cada `memory_entry` grava os totais de síntese e a lista determinística de digests utilizados, permitindo que recalls futuros saibam exatamente quais planos/provas/programas foram gerados em cada execução.
- Para consumir esses snapshots em código, use `nsr.meta_memory_to_dict(meta_memory_node)`, que devolve `{"size": ..., "digest": ..., "entries": [...]}` com todos os campos persistidos (incluindo `synthesis_*`). Isso facilita pipelines externos que dependem do histórico determinístico sem ter que reimplementar parsing LIU.
- Operadores Φ adicionados continuamente: recentes incluem `Φ_PLAN_DECOMPOSE`, que transforma objetivos textuais em passos determinísticos (`plan_step`) para alimentar reasoning/memória com rastros auditáveis.
...
- `src/nsr/` – maior pacote:
  ...
  - `bayes_engine.py`, `markov_engine.py`, `regression_engine.py`, `factor_graph_engine.py`, `polynomial_engine.py` – bibliotecas sem pesos (redes bayesianas, cadeias/hmms, regressões lineares, grafos fatoriais e fatoração polinomial). Seus bridges textuais (`BAYES`, `MARKOV`, `REGRESS`, `FACTOR`, `POLY`) alimentam as rotas `STAT`/`MATH`.
