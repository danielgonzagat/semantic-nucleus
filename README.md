...
- `python -m nsr.cli "...código..." --include-meta --expect-code-digest HEX` garante que o `code_summary_digest` (hash Blake2b do AST resumido) corresponde ao valor esperado; use `--include-code-summary` para incluir o resumo de código serializado no payload principal sem precisar do pacote meta completo.
