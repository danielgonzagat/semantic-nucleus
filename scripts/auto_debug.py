...
2. pytest (suíte completa)
3. pytest tests/cts (compatibilidade de protocolos)
4. Validação dos LangPacks (pt/en/es/fr/it)
5. metanucleus-auto-evolve all --apply (gera patches determinísticos)
...
DEFAULT_DIAGNOSTICS: tuple[CommandSpec, ...] = (
    CommandSpec(name="pre-commit", argv=("pre-commit", "run", "--all-files")),
    CommandSpec(name="pytest", argv=("python", "-m", "pytest")),
    CommandSpec(name="pytest-cts", argv=("python", "-m", "pytest", "tests/cts")),
) + tuple(
    CommandSpec(
        name=f"langpack-{code}",
        argv=("python", "scripts/langpack_check.py", "--code", code, "--fail-on-warn"),
        critical=False,
    )
...
DEFAULT_FIXERS: tuple[CommandSpec, ...] = (
    CommandSpec(
        ...
        argv=("metanucleus-auto-evolve", "all", "--apply"),
        critical=False,
    ),
)
...
class AutoDebugPipeline:
    ...
    def run(self) -> PipelineReport:
        ...
        for idx in range(1, self.max_cycles + 1):
            ...
            diagnostics_ok = all(result.ok for result in cycle.diagnostics if spec_is_critical(result, self.diagnostics))
            if diagnostics_ok:
                cycle.success = True
                cycles.append(cycle)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Pipeline determinístico de auto-debug.")
    parser.add_argument(
        "--max-cycles",
        type=int,
        default=3,
