...
- Conversão estruturada de código: `nsr.code_ast.build_python_ast_meta` serializa ASTs Python para LIU (`code_ast`), o `code_bridge` anexa o nó ao contexto e o MetaTransformer inclui o mesmo (inclusive em fallback textual quando o detector identifica código), expondo `code_ast_*` no `meta_summary`/CLI com contagem de nós, linguagem e flag de truncamento.
