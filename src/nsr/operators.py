...
def _op_explain(isr: ISR, args: Tuple[Node, ...], _: SessionCtx) -> ISR:
    ...
    report = _build_explanation_report(isr, focus)
...
def _build_explanation_report(isr: ISR, focus: Node) -> str:
    relations_preview = _relations_summary(isr.relations)
    context_preview = _context_summary(isr.context)
    ops_preview = _ops_summary(isr.ops_queue)
...
def _relations_summary(relations: Tuple[Node, ...], limit: int = 4) -> str:
    if not relations:
        return "sem relações"
    phrases = [_render_relation(rel) or rel.label or "REL" for rel in relations[:limit]]
...
def _context_summary(context: Tuple[Node, ...], limit: int = 3) -> str:
    if not context:
...
def _ops_summary(ops_queue: Iterable[Node], limit: int = 4) -> str:
...
def _struct_sentence(node: Node) -> str:
...
def _render_struct(node: Node) -> Node:
    return text(_struct_sentence(node))
